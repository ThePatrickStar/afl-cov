...
def process_fot_test_cases(cargs):
    ...
    while True:
        ...
        if not import_fuzzing_dirs(cov_paths, cargs):
            danger("# No fuzzing dir imported!")
            rv = False
            break
        ...
        for fuzz_dir in cov_paths['dirs']:
            info("# Checking the dir: %s" % fuzz_dir)
            do_break = False
            last_file = False
            num_files = 0
            new_files = []
...
def id_range_update(fot_file, cov_paths):
    id_val = int(os.path.basename(fot_file).split(',')[0].split(':')[1])
...
def coverage_diff(cycle_num, fuzz_dir, cov_paths, fot_file, cov, cargs):
    log_lines = []
    delta_log_lines = []
    ...
    delta_file = b_file = os.path.basename(fot_file)
    ...
    # We aren't interested in the number of times FOT has executed
    # a line or function (since we can't really get this anyway because
    # gcov stats aren't influenced by FOT directly) - what we want is
...
def write_pos_cov(pos_cov, cov_paths, cargs):
    ...
    cfile.write("# least one FOT test case. See the cov/id-delta-cov file\n")
...
def write_status(status_file):
    ...
    f.write("fot_cov_pid     : %d\n" % os.getpid())
    f.write("fot_cov_version : %s\n" % __version__)
...
def search_cov(cargs):
    ...
    id_delta_file = cargs.fot_fuzzing_dir + '/cov/id-delta-cov'
    log_file = cargs.fot_fuzzing_dir + '/cov/fot-cov.log'
...
def get_cycle_num(id_num, cargs):
    ...
    if not is_dir(cargs.fot_fuzzing_dir + '/plot_data'):
        return cycle_num

    with open(cargs.fot_fuzzing_dir + '/plot_data') as f:
        for line in f:
...
def is_fot_fuzz_running(cargs):
    pid = None
    stats_file = cargs.fot_fuzzing_dir + '/fuzzer_stats'

    if os.path.exists(stats_file):
        pid = get_running_pid(stats_file, 'fuzzer_pid\s+\:\s+(\d+)')
    else:
        for p in os.listdir(cargs.fot_fuzzing_dir):
            stats_file = "%s/%s/fuzzer_stats" % (cargs.fot_fuzzing_dir, p)
            if os.path.exists(stats_file):
                # allow a single running FOT instance in parallel mode
                # to mean that FOT is running (and may be generating
                # new code coverage)
                pid = get_running_pid(stats_file, 'fuzzer_pid\s+\:\s+(\d+)')
                if pid:
                    break
...
def run_cmd(cmd, log_file, cargs, collect):
    ...
    if (es != 0) and (collect == LOG_ERRORS):
        if log_file:
            ...
            warn("    Non-zero exit status '%d' for CMD: %s" % (es, cmd))
...
def import_fuzzing_dirs(cov_paths, cargs):
    if not cargs.fot_fuzzing_dir:
        danger("[*] Must specify FOT fuzzing dir with --fot-fuzzing-dir or -d")
        return False
    ...
    for line in run_cmd("%s -a %s" % (cargs.readelf_path, binary),
        ...
        if ' __gcov' in line:
            if cargs.validate_args or cargs.gcov_check or cargs.gcov_check_bin:
                info("[+] Binary '%s' is compiled with code coverage support gcc." % binary)
            rv = True
            break

        if '__llvm_gcov' in line:
            if cargs.validate_args or cargs.gcov_check or cargs.gcov_check_bin:
                info("[+] Binary '%s' is compiled with code coverage support via llvm." % binary)
            rv = True
            break

    if not rv and cargs.gcov_check_bin:
        warn("[*] Binary '%s' is not compiled with code coverage support." % binary)
...
def check_requirements(cargs):
    ...
    if lcov is None or gcov is None:
        danger("Required command not found :")
    else:
    ...
    if lcov is None:
        danger("[*] lcov command does not exist : %s" % cargs.lcov_path)
    if genhtml is None and not cargs.disable_lcov_web:
        danger("[*] genhtml command does not exist : %s" % cargs.genhtml_path)
    if gcov is None:
        danger("[*] gcov command does not exist : %s" % cargs.gcov_path)
...
def is_gcov_enabled(cargs):
    if not is_exe(cargs.readelf_path):
        danger("[*] Need a valid path to readelf, use --readelf-path")
        return False

    if cargs.coverage_cmd:
        if 'FOT_FILE' not in cargs.coverage_cmd:
            danger("[*] --coverage-cmd must contain FOT_FILE")
            return False
        ...
        if not found_exec:
            danger("[*] Could not find an executable binary "
                  "--coverage-cmd '%s'" % cargs.coverage_cmd)
            return False

        if not cargs.disable_gcov_check and not found_code_cov_binary:
            danger("[*] Could not find an executable binary with code "
        ...
        danger("[*] Either --coverage-cmd or --gcov-check-bin required in --gcov-check mode")
        return False
...
def validate_cargs(cargs):
    if cargs.coverage_cmd:
        ...
        if not cargs.func_search and not cargs.line_search:
            danger("[*] Must set --coverage-cmd or --func-search/--line-search")
            return False

    if cargs.code_dir:
        if not is_dir(cargs.code_dir):
            danger("[*] --code-dir path does not exist")
            return False
        ...
        if not cargs.func_search and not cargs.line_search:
            danger("[*] Must set --code-dir unless using --func-search "
                  "against existing fot-cov directory")
            return False

    if cargs.func_search or cargs.line_search:
        if not cargs.fot_fuzzing_dir:
            danger("[*] Must set --fot-fuzzing-dir")
            return False
        if cargs.func_search and '()' not in cargs.func_search:
            cargs.func_search += '()'
        if cargs.line_search and not cargs.src_file:
            danger("[*] Must set --src-file in --line-search mode")
            return False
    ...
    if not cargs.live and not is_dir(cargs.fot_fuzzing_dir):
        danger("[*] It doesn't look like directory '%s' exists"
              % cargs.fot_fuzzing_dir)
        return False

    if cargs.disable_lcov_web and cargs.lcov_web_all:
        danger("[*] --disable-lcov-web and --lcov-web-all are incompatible")
        return False
...
def gcno_files_exist(cargs):
    ...
    if not found_code_coverage_support:
        danger("[*] Could not find any *.gcno files in --code-dir "
...
def is_fot_running(cargs):
    while not is_dir(cargs.fot_fuzzing_dir):
        if not cargs.background:
            warn("[-] Sleep for %d seconds for FOT fuzzing directory to be created..."
                  % cargs.sleep)
        time.sleep(cargs.sleep)

    # if we make it here then fot-fuzz is presumably running
    while not is_fot_fuzz_running(cargs):
        if not cargs.background:
            warn("[-] Sleep for %d seconds waiting for fot-fuzz to be started...."
                  % cargs.sleep)
        time.sleep(cargs.sleep)
    return
...
def stop_fot(cargs):
    ...
    # note that this function only looks for fot-fuzz processes - it does not
    # stop fot-cov processes since they will stop on their own after fot-fuzz
    # is also stopped.
    ...
    if not is_dir(cargs.fot_fuzzing_dir):
        danger("[*] Doesn't look like FOT fuzzing directory '%s' exists."
              % cargs.fot_fuzzing_dir)
        return False

    if os.path.exists(cargs.fot_fuzzing_dir + '/fuzzer_stats'):
        fot_pid = get_running_pid(cargs.fot_fuzzing_dir + '/fuzzer_stats',
                                  'fuzzer_pid\s+\:\s+(\d+)')
        if fot_pid:
            info("[+] Stopping running fot-fuzz instance, PID: %d" % fot_pid)
            os.kill(fot_pid, signal.SIGTERM)
        else:
            info("[-] No running fot-fuzz instance")
            rv = False
    else:
        found = False
        for p in os.listdir(cargs.fot_fuzzing_dir):
            stats_file = cargs.fot_fuzzing_dir + '/' + p + '/fuzzer_stats'
            if os.path.exists(stats_file):
                fot_pid = get_running_pid(stats_file, 'fuzzer_pid\s+\:\s+(\d+)')
                if fot_pid:
                    info("[+] Stopping running fot-fuzz instance, PID: %d"
                          % fot_pid)
                    os.kill(fot_pid, signal.SIGTERM)
                    found = True
        if not found:
            warn("[-] No running fot-fuzz instance")
            rv = False
...
def check_core_pattern():
    ...
    # check /proc/sys/kernel/core_pattern to see if fot-fuzz will
    # accept it
    if os.path.exists(core_pattern_file):
        with open(core_pattern_file, 'r') as f:
            if f.readline().rstrip()[0] == '|':
                # same logic as implemented by fot-fuzz itself
                warn("[*] fot-fuzz requires 'echo core >%s'"
                      % core_pattern_file)
                rv = False
    return rv
